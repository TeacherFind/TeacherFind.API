...
public class AuthService : IAuthService
{
    ...
    public async Task<User> RegisterAsync(RegisterRequest request)
    {
        ...
        Console.WriteLine("============================================================");
        Console.WriteLine($"[EMAIL VERIFICATION CODE] {user.Email} -> {emailCode.Code}");
        Console.WriteLine("============================================================");
        ...
        try
        {
            await _emailService.SendAsync(
                ...
                """,
                $"E-posta doğrulama kodunuz: {emailCode.Code}");
        }
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Doğrulama e-postası gönderilemedi ancak kullanıcı kaydı tamamlandı. UserId: {UserId}, Email: {Email}",
                user.Id,
                user.Email);

            Console.WriteLine("============================================================");
            Console.WriteLine("[MAIL HATASI AMA KAYIT TAMAMLANDI]");
            Console.WriteLine($"Email: {user.Email}");
            Console.WriteLine($"Kod: {emailCode.Code}");
